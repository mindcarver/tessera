...
If a layer's instruction requires subagents and none are available, generate one review prompt file per such layer in `/Users/carver/workspace/mindcarver/tessera/_bmad-output/implementation-artifacts` and HALT. Ask the human to run each in a separate session and paste back the findings.
...
- **defer** — pre-existing issue not caused by this change. Append one new entry to `/Users/carver/workspace/mindcarver/tessera/_bmad-output/implementation-artifacts/deferred-work.md` using this format. Do not modify existing entries or look for duplicates.
  ```markdown
