...
includedFiles:
  ...
  epics:
    - epics.md
  ux:
    - epics.md#ux-design-requirements
---
...
**Selected canonical document set:**
...
The folder also contains reconciliation and review support material. Those
files are evidence and review history, not competing canonical PRD versions.
No whole-versus-sharded duplicate was selected.
...
The sibling reconciliation, research, and review files are supporting
artifacts rather than alternate architecture sources. No canonical duplicate
was found.
...
No standalone UX design document was found. The approved UX decisions are
embedded in `epics.md` as UX-DR1 through UX-DR14 and are the selected UX input
for this assessment.

### Discovery Issues and Resolution

- **Duplicates:** None.
- **Missing standalone artifact:** UX design document.
- **Resolution:** User confirmed with `C` that the embedded UX decisions in
  `epics.md` are the UX source for this readiness assessment.
...
user explicitly approved `epics.md` UX-DR1 through UX-DR14 as the UX input for
this assessment.
...
separate domains, the Vault zero-write boundary is explicit, and no
future-Epic dependency blocks Epic 6.
...
decomposition, not missing product intent.
...
1. **Refine Story 6.1:** separate or explicitly checkpoint persisted
   ...
   protection.
2. **Split Story 6.2:** separate Knowledge schema/migration/identity from the
   read-only enumerate/parse/index pipeline and its zero-mutation evidence.
3. **Split and make Story 6.3 measurable:** separate Knowledge Inventory from
   watcher/reconcile/rebind/recovery; move cadence threshold locking to the
   performance gate and keep the functional Story bounded and deterministic.
...
2. Add the Rust-owned native Vault-picker interaction contract: the browser
   submits only an action request, and Rust returns a validated existing Vault
   Candidate rather than an arbitrary path or directory handle.
3. Name the decision/evidence artifacts for the per-note byte ceiling and
   Phase C.0 performance gate. Keep raw note bodies, private filenames, Vault
   paths, and registry payloads out of committed benchmark output.
4. Add a focused UX contract for Vault onboarding, Knowledge Inventory,
   multi-Vault filtering, loading/error states, keyboard and screen-reader
   behavior, and Open-in-Obsidian dispatch versus visible-success feedback.
...
- **UX/evidence/documentation concerns:** 8
- **Missing FR coverage:** 0
- **Architecture linter findings from the approved planning pass:** 0

### Final Note

The Obsidian direction itself is sound and remains strictly read-only. The
planning set is not yet implementation-ready because the new work is packaged
into Stories that are too broad or whose completion boundary depends on later
measurement. Refining those Stories now is materially cheaper than discovering
the same ambiguity during migration, watcher, or performance implementation.
