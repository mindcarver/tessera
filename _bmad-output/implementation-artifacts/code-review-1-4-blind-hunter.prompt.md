# Blind Hunter — Tessera Story 1.4

Work in `/Users/carver/workspace/mindcarver/tessera`. Do not modify files.

Invoke the `bmad-review` skill with only the `adversarial` lens on the supplied review material.
...
Return only substantiated findings as a Markdown list. For every finding include: severity, exact `file:line`, violated requirement or invariant, a concise failure scenario, and concrete evidence. Do not report stylistic suggestions.
